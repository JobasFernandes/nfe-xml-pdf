...
class Danfe(xFPDF):
    ...
    def __init__(self, xmls=None, image=None, cfg_layout="ICMS_ST", receipt_pos="top"):
        ...
        for xml in xmls:
            ...
            nr_lin_pg_1 = 23
            nr_lin_pg = 70
            self.lin_emit = 31 if self.receipt_pos == "top" else 10
            self.lin_prod = 161 if self.receipt_pos == "top" else 140
    ...
    def produtos_l(self, paginator=None, list_desc=None):
        ...
        self.set_font("Times", "B", 7)
        self.text(x=11, y=self.lin_prod - 1, txt="DADOS DO PRODUTO/SERVIÇO")
        self.rect(x=10, y=self.lin_prod, w=248, h=h_produtos, style="")
        ...
        # Colunas
        self.set_font("Times", "B", 5)
        self.line(10, self.lin_prod + 6, 284, self.lin_prod + 6)
        col_ = 10  # 10 mm margem
        self.set_xy(x=10, y=self.lin_prod + 1)
        for id, col in enumerate(cols):
            col_ += col
        ...
        for id in range(paginator[0], paginator[1]):
            ...
            if self.get_y() < (self.lin_prod + h_produtos):  # id % 2:
                y = self.get_y() - 0.1
                self.dashed_line(10, y, 284, y, dash_length=1, space_length=1)
